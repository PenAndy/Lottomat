...
namespace Lottomat.SOA.API.Controllers
{
    ...
    {
        private static readonly AppKeyBLL appKeyBll = new AppKeyBLL();
            ...
            Logger(typeof(OAuthController), "根据AppKey获取Token-GetToken", () =>
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        AppKeyEntity appKeyEntity = appKeyBll.GetEntity(a => a.AppKey.Equals(appkey));
                        if (appKeyEntity != null)
                        {
                            ...
                            if (appKeyEntity.AppSecret.Equals(appsecret))
                            {
                                ...
                                if (token == null)
                                {
                                    ...
                                    DateTime time =
                                        DateTimeHelper.Now
                                            .AddDays(GlobalStaticConstant.TOKEN_EXPIRE_TIME);
        ...
        private string GetSignToken(string appkey)
        {
            ...
            //第二次
            string sec = DESEncrypt.Encrypt(first);

            //第三次
            string last = DESEncrypt.Encrypt(Md5Helper.MD5(sec, 32));
            //appkey+token降序排序
            string res = string.Concat((appkey + last).OrderBy(o => o));

            string temp = DESEncrypt.Encrypt(Md5Helper.MD5(res, 32));
            return temp;
        }
