...
namespace Lottomat.SOA.API.Controllers.V1
{
    ...
    /// 高频彩11选5
    /// </summary>
    public class GPC11X5Controller : BaseApiController
    {
        /// <summary>
        /// 公共BLL
        /// </summary>
        ...
        #region 获取高频彩11选5彩种历史记录
        /// <summary>
        /// 获取高频彩11选5彩种历史记录
        /// </summary>
        /// <returns></returns>
        [HttpPost]
        public HttpResponseMessage GetGPC11X5HistoryLotteryList(HistoryLotteryArgEnyity arg)
        {
            BaseJson<string> resultMsg = new BaseJson<string> { Status = (int)JsonObjectStatus.Error, Message = "服务器未知错误。", Data = null };

            Logger(typeof(GPC11X5Controller), arg.TryToJson(), "获取高频彩11选5彩种历史记录-GetGPC11X5HistoryLotteryList", () =>
            {
        ...
        private string GetResultByEnumCode(SCCLottery type, HistoryLotteryArgEnyity arg)
        {
            ...
            switch (type)
            {
                case SCCLottery.HeBei11X5:
                    res = AppendCommonResult(data, SCCLottery.HeBei11X5);
                    break;
                default:
