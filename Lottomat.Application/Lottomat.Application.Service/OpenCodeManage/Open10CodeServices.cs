...
namespace Lottomat.Application.Service.OpenCodeManage
{
    ...
    /// 开奖10个球号的彩种数据服务
    /// </summary>
    public class Open10CodeServices : RepositoryFactory, IOpen10Code
    {
        /// <summary>
        /// 获取最新一条记录
        /// </summary>
        ...
        public OpenCode10Model GetLastItem(SCCLottery lottery)
        {
            ...
            {
                var result = BaseServices.LoadData<OpenCode10Model>(ds.DataSet.Tables[0].Rows[0]);
                return result;
            }
            return null;
        }
        ...
        private static string AddItemSql = @"IF NOT EXISTS(SELECT TOP 1 1 FROM {0} WHERE Term = @Term)
                                            BEGIN
	                                            INSERT INTO {0}(Term,OpenCode1,OpenCode2,OpenCode3,OpenCode4,OpenCode5,OpenCode6,OpenCode7,OpenCode8,OpenCode9,OpenCode10,OpenTime,Addtime,ID)
                                                SELECT @Term,@OpenCode1,@OpenCode2,@OpenCode3,@OpenCode4,@OpenCode5,@OpenCode6,@OpenCode7,@OpenCode8,@OpenCode9,@OpenCode10,@OpenTime,GETDATE(),@ID
                                            END";
        #endregion
    }
}
