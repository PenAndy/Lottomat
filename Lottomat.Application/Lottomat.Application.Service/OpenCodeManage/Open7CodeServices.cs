...
namespace Lottomat.Application.Service.OpenCodeManage
{
    ...
    /// 开奖7个球号的彩种数据服务
    /// </summary>
    public class Open7CodeServices : RepositoryFactory, IOpen7Code
    {
        /// <summary>
        /// 获取最新一条记录
        /// </summary>
        ...
        private static string AddItemSql = @"IF NOT EXISTS(SELECT TOP 1 1 FROM {0} WHERE Term = @Term)
                                            BEGIN
	                                            INSERT INTO {0}(Term,OpenCode1,OpenCode2,OpenCode3,OpenCode4,OpenCode5,OpenCode6,OpenCode7,OpenTime,Addtime,ID)
                                                SELECT @Term,@OpenCode1,@OpenCode2,@OpenCode3,@OpenCode4,@OpenCode5,@OpenCode6,@OpenCode7,@OpenTime,GETDATE(),@ID
                                            END";
        #endregion
    }
}
