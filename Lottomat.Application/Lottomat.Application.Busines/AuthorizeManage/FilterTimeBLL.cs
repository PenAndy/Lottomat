    ...
    public class FilterTimeBLL
    {
        ...
        public bool FilterTime()
        {
            ...
            if (cacheList == null)
            {
                ...
                CacheFactory.Cache().WriteCache(filterTimeList, cacheKey, DateTimeHelper.Now.AddMinutes(1));
            }
            else
            {
                filterTimeList = cacheList;
            }
            int weekday = DateTimeHelper.GetNumberWeekDay(DateTimeHelper.Now);
            string time = DateTimeHelper.Now.ToString("HH") + ":00";
