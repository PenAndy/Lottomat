...
namespace Lottomat.Application.Admin.Areas.PublicInfoManage.Controllers
{
    /// <summary>
    /// 版 本 1.0
    ...
    public class ResourceFileController : MvcControllerBase
    {
        ...
        /// <param name="keyValue">主键值</param>
        /// <param name="fileType">文件类型</param>
        /// <param name="isShare">是否共享：1-共享 0-取消共享</param>
        /// <returns></returns>
        [HttpPost]
        [ValidateAntiForgeryToken]
        [AjaxOnly]
        ...
        public ActionResult UploadifyFile(string folderId, HttpPostedFileBase filedata)
        {
            try
            {
                ...
                string userId = OperatorProvider.Provider.Current().UserId;
                string fileGuid = CommonHelper.GetGuid().ToString();
                long filesize = filedata.ContentLength;
                string fileEextension = Path.GetExtension(filedata.FileName);
                string uploadDate = DateTimeHelper.Now.ToString("yyyyMMdd");
                ...
                if (!System.IO.File.Exists(fullFileName))
                {
                    ...
                    fileInfoEntity.FolderId = !string.IsNullOrEmpty(folderId) ? folderId : "0";
        ...
        {
            FileInfoEntity data = fileInfoBLL.GetEntity(keyValue);
            if (data != null)
            {
