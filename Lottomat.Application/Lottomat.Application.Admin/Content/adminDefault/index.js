...
loadnav = function () {
    ...
    $('#nav li a').click(function () {
        ...
        if (data.Target == "iframe") {
            tablist.newTab({
                ...
                closed: true,
                icon: data.Icon,
                url: contentPath + data.UrlAddress
            });
        }
    });
    ...
    $("#nav li.sub").hover(function (e) {
        ...
        if (top + height > wheight) {
            $ul.css('top', parseInt('-' + (height - 180))).css('left', '130px');
        } else {
    ...
    //三级菜单鼠标hover
    $("#nav ul.sub-nav.sub2 li").hover(function (e) {

        var $sub = $(this).find('.sub-child ul');
        $sub.show();

        var top = $(this).find('.sub-child ul').offset().top;
        var height = $sub.height();
        var wheight = $(window).height();
        if (top + height > wheight) {
            $sub.css('top', parseInt('-' + (height - 200))).css('left', '130px');
        } else {
            $sub.css('top', '0px').css('left', '130px');
        }
    }, function (e) {
        $(this).find('.sub-child ul').hide();
        $(this).find('.sub-child ul').css('top', '0px');
    });

    //导航二菜单
    function getsubnav(moduleId) {
        var _html = '<div class="sub-nav-wrap">';
        _html += '<ul class="sub-nav sub1">';
        $.each(authorizeMenuData, function (i) {
            var row = authorizeMenuData[i];
            if (row.ParentId == moduleId) {
                if (isbelowmenu(row.ModuleId) == 0) {
                    ...
                    _html += getsubchildnav(row.ModuleId);
                    _html += '</li>'; //sub
                }
                navJson[row.ModuleId] = row;
            }
    ...
    //导航三级菜单
    function getsubchildnav(moduleId) {
        var _html = '';//<div class="sub-nav-wrap">
        _html += '<ul class="sub-nav sub2">';
        $.each(authorizeMenuData, function (i) {
            var row = authorizeMenuData[i];
            if (row.ParentId == moduleId) {
                if (isbelowmenu(row.ModuleId) == 0) {
