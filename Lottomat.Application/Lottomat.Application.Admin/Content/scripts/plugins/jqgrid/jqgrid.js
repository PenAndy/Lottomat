        ...
        return this.each(function () {
            if (!this.grid) {
                var d = b.extend(!0, {
                    ...
                    rowTotal: null,
                    records: 0,//只读属性，定义了返回的记录数
                    pager: "",//定义翻页用的导航栏，必须是有效的html元素。翻页工具栏可以放置在html页面任意位置
                    pgbuttons: !0,//是否显示翻页按钮
                    pginput: !0,//是否显示跳转页面的输入框
                    ...
                    datatype: "xml",//从服务器端返回的数据类型，默认xml。可选类型：xml，local，json，jsonnp，script，xmlstring，jsonstring，clientside
                    mtype: "GET",//ajax提交方式。POST或者GET，默认GET
                    altRows: !1,//设置表格 zebra-striped 值
                    selarrrow: [],//只读属性，用来存放当前选择的行
                    savedRow: [],//只读属性，只用在编辑模式下保存数据
                    shrinkToFit: !0,//此属性用来说明当初始化列宽度时候的计算类型，如果为ture，则按比例初始化列宽度。如果为false，则列宽度使用colModel指定的宽度
                    xmlReader: {},
                    jsonReader: {},
                    subGrid: !1,
                    subGridModel: [],
                    ...
                    onHeaderClick: null,
                    viewrecords: !1,//定义是否要显示总记录数
                    loadonce: !1,//如果为ture则数据只从服务器端抓取一次，之后所有操作都是在客户端执行，翻页功能会被禁用
                    multiselect: !1,//定义是否可以多选
                    multikey: !1,//只有在multiselect设置为ture时起作用，定义使用那个key来做多选。shiftKey，altKey，ctrlKey
                    editurl: null,
                    search: !1,
                    caption: "",//表格名称
                    hidegrid: !0,
                    ...
                    ExpandColumn: null,
                    tree_root_level: 0,
                    prmNames: {//默认: {page:“page”,rows:“rows”, sort: “sidx”,order: “sord”, search:“_search”, nd:“nd”, npage:null} 当参数为null时不会被发到服务器端
                        page: "page",
                        rows: "rows",
                        sort: "sidx",
                        order: "sord",
                    ...
                    forceFit: !1,
                    gridstate: "visible",
                    cellEdit: !1,//启用或者禁用单元格编辑功能
                    cellsubmit: "remote",//定义了单元格内容保存位置
                    nv: 0,
                    loadui: "enable",
                    toolbar: [!1, ""],//表格的工具栏。数组中有两个值，第一个为是否启用，第二个指定工具栏位置（相对于body layer），如：[true,”both”] 。工具栏位置可选值：“top”,”bottom”, “both”. 如果工具栏在上面，则工具栏id为“t_”+表格id；如果在下面则为 “tb_”+表格id；如果只有一个工具栏则为 “t_”+表格id
                    scroll: !1,//创建一个动态滚动的表格，当为true时，翻页栏被禁用，使用垂直滚动条加载数据，且在首次访问服务器端时将加载所有数据到客户端。当此参数为数字时，表格只控制可见的几行，所有数据都在这几行中加载
                    multiboxonly: !1,//只有当multiselect = true.起作用，当multiboxonly 为ture时只有选择checkbox才会起作用
                    deselectAfterSort: !0,
                    scrollrows: !1,//当为true时让所选择的行可见
                    autowidth: !1,//如果为ture时，则当表格在首次被创建时会根据父元素比例重新调整表格宽度。如果父元素宽度改变，为了使表格宽度能够自动调整则需要实现函数：setGridWidth
                    scrollOffset: 18,//设置垂直滚动条宽度
                    cellLayout: 5,//定义了单元格padding + border 宽度。通常不必修改此值。初始值为5
                    subGridWidth: 20,
                    multiselectWidth: 20,
                    gridview: !1,
                    rownumWidth: 30,//如果rownumbers为true，则可以设置column的宽度
                    rownumbers: !1,//如果为ture则会在表格左边新增一列，显示行顺序号，从1开始递增。此列名为'rn'.
